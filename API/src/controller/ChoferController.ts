...
class ChoferController {
  ...
  static add = async (req: Request, resp: Response) => {
    try {
      ...
      // Validación de fecha si es mayor o menor de edad

      const fechaNacimiento: Date = new Date(fechaNac);
      const fechaActual: Date = new Date();

      const fechaCumple18: Date = new Date(
        fechaNacimiento.getFullYear() + 18,
        fechaNacimiento.getMonth(),
        fechaNacimiento.getDate()
      );

      if (fechaActual < fechaCumple18) {
        return resp.status(403).json({
          mensaje: 'El chofer debe tener al menos 18 años para continuar'
        });
      }
